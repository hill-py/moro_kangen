...
{
    $stmt = $db->prepare(
        "SELECT id_pesanan, status FROM pesanan WHERE id_pesanan = ? LIMIT 1"
    );
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isKaryawan()) {
        ...
        if (!$pesanan) {
            ...
            $statusValid = in_array($status, ['dibayar', 'selesai'], true);
            $transisiValid =
                ($pesanan['status'] === 'menunggu' && $status === 'dibayar') ||
                ($pesanan['status'] === 'dibayar'  && $status === 'selesai');

            if (!$statusValid || !$transisiValid) {
                    ...
                    if ($status === 'selesai') {
                        ...
                        if (!empty($kursiIds)) {
                            $stmtKursi = $db->prepare(
                                "UPDATE kursi SET status = 'kosong' WHERE id_kursi = ?"
                            );
                            foreach ($kursiIds as $idKursi) {
                                $stmtKursi->bind_param('i', $idKursi);
                                $stmtKursi->execute();
                            }
                            $stmtKursi->close();

                            $stmtDel = $db->prepare(
                                "DELETE FROM detail_kursi WHERE id_pesanan = ?"
                            );
                            $stmtDel->bind_param('i', $idPesanan);
                            $stmtDel->execute();
                            $stmtDel->close();
                        }
                    }
...
if (isset($_GET['success'])) {
    ...
    } elseif ($successType === 'status') {
        $message = 'Status pesanan berhasil diperbarui.';
    }
}

/* ---------------------------------------------------------------
   Ambil daftar pesanan aktif (bukan selesai)
--------------------------------------------------------------- */
$orders = [];
$result = $db->query("
    SELECT
...
// Ambil item detail untuk semua pesanan sekaligus
if (count($orders) > 0) {
                            ...
                            <td>
                                <?php foreach ($order['items'] as $item): ?>
                                    <div class="order-item-line">
                                        <strong>
                                            <?= (int) $item['jumlah'] ?>x <?= h($item['nama_menu']) ?>
                                        </strong>
                                        <?php if ($item['catatan'] !== ''): ?>
                                ...
                                <?php if (isKaryawan()): ?>
                                    <div class="td-actions">
                                        ...
                                        <?php if ($order['status'] === 'menunggu'): ?>
                                            <a href="../pelanggan/index.php?edit=<?= (int) $order['id_pesanan'] ?>"
                                               class="btn btn-primary btn-sm">
                                                Edit
                                            </a>
                                            <a href="../pembayaran/index.php?bayar=<?= (int) $order['id_pesanan'] ?>"
